...
const requestedVariant = variantIndex >= 0 ? String(args[variantIndex + 1] || '') : 'original';
const variant = requestedVariant === 'normal' ? 'normal' : 'original';
const variantSuffix = variant === 'normal' ? '_normal' : '';
const frameDir = path.join(root, `_frames_${tier}${variantSuffix}`);
...
const duration = variant === 'normal' ? 7.9 : 5.2;
...
(async () => {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), `f009-v7-critical-${variant}-preview-`));
  ...
  try {
    ...
    if (singleTime !== null && Number.isFinite(singleTime)) {
      ...
      process.stdout.write(`SINGLE ${tier} ${variant} ${singleTime.toFixed(3)} ${outputPath}\n`);
    } else {
      for (let i = 0; i < frameCount; i += 1) {
        ...
        if (i % Math.max(1, Math.round(fps / 3)) === 0 || i === frameCount - 1) {
          process.stdout.write(`FRAME ${tier} ${variant} ${i + 1}/${frameCount} t=${time.toFixed(2)}\n`);
        }
      }
    }
