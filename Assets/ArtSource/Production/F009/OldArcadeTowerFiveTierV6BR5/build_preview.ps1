...
foreach ($tier in $tiers) {
  ...
  $audio = Join-Path $root ("f009_v6b_r5_tier_" + $tier.Slug + "_temp_audio_20260722.wav")
  $video = Join-Path $root ("f009_v6b_r5_tier_" + $tier.Slug + "_preview_20260722.mp4")
  $poster = Join-Path $root ("f009_v6b_r5_tier_" + $tier.Slug + "_peak_20260722.png")
  $result = Join-Path $root ("f009_v6b_r5_tier_" + $tier.Slug + "_result_20260722.png")
...
$syncVideo = Join-Path $root 'f009_v6b_r5_five_tier_sync_visual_comparison_20260722.mp4'
$sequentialVideo = Join-Path $root 'f009_v6b_r5_five_tier_sequential_audio_comparison_20260722.mp4'
$peakBoard = Join-Path $root 'f009_v6b_r5_five_tier_peak_review_board_20260722.png'
$resultBoard = Join-Path $root 'f009_v6b_r5_five_tier_result_review_board_20260722.png'
$towerBoard = Join-Path $root 'f009_v6b_r5_five_tier_tower_crop_review_20260722.png'
...
$towerFilters = @()
for ($i = 0; $i -lt $tiers.Count; $i++) {
  $towerFilters += "[$i`:v]crop=340:540:0:150,scale=380:604,pad=384:648:2:42:color=0x080705,drawtext=fontfile='$fontFile':text='$($tiers[$i].Label)':fontcolor=0xF1A02D:fontsize=23:x=14:y=7[t$i]"
}
$towerGraph = ($towerFilters -join ';') + ';[t0][t1][t2][t3][t4]hstack=inputs=5[outv]'
& $ffmpeg -hide_banner -loglevel error -y `
  -i $posterPaths[0] -i $posterPaths[1] -i $posterPaths[2] -i $posterPaths[3] -i $posterPaths[4] `
  -filter_complex $towerGraph -map '[outv]' -frames:v 1 -update 1 $towerBoard
if ($LASTEXITCODE -ne 0) { throw 'Tower crop review board build failed.' }
...
$outputs = @($syncVideo, $sequentialVideo, $peakBoard, $resultBoard, $towerBoard) + $videoPaths
